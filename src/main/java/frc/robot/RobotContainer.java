...
import frc.robot.subsystems.collectionSubsystem.IntakeExtensionIO;
import frc.robot.subsystems.collectionSubsystem.IntakeExtensionIOReal;
import frc.robot.subsystems.collectionSubsystem.IntakeRollers;
import frc.robot.subsystems.collectionSubsystem.IntakeRollersIO;
import frc.robot.subsystems.collectionSubsystem.IntakeRollersIOReal;
import frc.robot.subsystems.drive.CommandSwerveDrivetrain;
import frc.robot.subsystems.drive.vision.Vision;
import frc.robot.subsystems.shooterSubsystem.Flywheel;
import frc.robot.subsystems.shooterSubsystem.FlywheelIO;
import frc.robot.subsystems.shooterSubsystem.FlywheelIOReal;
import frc.robot.subsystems.shooterSubsystem.Hood;
import frc.robot.subsystems.shooterSubsystem.HoodIO;
import frc.robot.subsystems.shooterSubsystem.HoodIOReal;
...
public class RobotContainer {
    ...
    private final Turret turret = new Turret(createTurretIO());
    private final Hood hood = new Hood(createHoodIO());
    private final Flywheel flywheel = new Flywheel(createFlywheelIO());
    private final IntakeExtension extension = new IntakeExtension(createIntakeExtensionIO());
    private final IntakeRollers rollers = new IntakeRollers(createIntakeRollersIO());
    private final Indexer indexer = new Indexer(createIndexerIO());
    ...
    private static TurretIO createTurretIO() {
        return RobotBase.isReal() ? new TurretIOReal() : new TurretIO() {};
    }

    private static HoodIO createHoodIO() {
        return RobotBase.isReal() ? new HoodIOReal() : new HoodIO() {};
    }

    private static FlywheelIO createFlywheelIO() {
        return RobotBase.isReal() ? new FlywheelIOReal() : new FlywheelIO() {};
    }

    private static IntakeExtensionIO createIntakeExtensionIO() {
        return RobotBase.isReal() ? new IntakeExtensionIOReal() : new IntakeExtensionIO() {};
    }

    private static IntakeRollersIO createIntakeRollersIO() {
        return RobotBase.isReal() ? new IntakeRollersIOReal() : new IntakeRollersIO() {};
    }

    private static IndexerIO createIndexerIO() {
        return RobotBase.isReal() ? new IndexerIOReal() : new IndexerIO() {};
    }
