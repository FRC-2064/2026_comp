...
public class TeleopDrive extends Command {
    ...
    private final Trigger inTrenchZone = new Trigger(this::inTrenchZone).debounce(0.1);
    private final Trigger inBumpZone = new Trigger(this::inBumpZone).debounce(0.1);

    private final PIDController trenchYController = new PIDController(4.0, 0, 0);
    private final PIDController rotationController = new PIDController(5.0, 0, 0.1);
    ...
    public TeleopDrive(
        ...
        this.xLimiter = new SlewRateLimiter(DriveConstants.MAX_ACCEL.in(MetersPerSecondPerSecond));
        this.yLimiter = new SlewRateLimiter(DriveConstants.MAX_ACCEL.in(MetersPerSecondPerSecond));
    ...
    private boolean poseInZone(Pose2d robotPose, double minX, double maxX, double minY, double maxY, double xExpansion, double yExpansion) {
        return robotPose.getX() + ROBOT_HALF_WIDTH >= minX - xExpansion &&
    ...
    private boolean inTrenchZone() {
        ...
        double trenchMinX = FieldConstants.LinesVertical.hubCenter - Units.inchesToMeters(60);
        double trenchMaxX = FieldConstants.LinesVertical.hubCenter + Units.inchesToMeters(60);

        // Opposing side X range
        double oppTrenchMinX = FieldConstants.LinesVertical.oppHubCenter - Units.inchesToMeters(60);
        double oppTrenchMaxX = FieldConstants.LinesVertical.oppHubCenter + Units.inchesToMeters(60);

        boolean inLeftTrench = poseInZone(robotPose,
            trenchMinX, trenchMaxX,
            FieldConstants.fieldWidth - FieldConstants.LeftTrench.openingWidth,
            FieldConstants.fieldWidth,
            ROTATION_EARLY_TRIGGER, 0.0);

        boolean inRightTrench = poseInZone(robotPose,
            trenchMinX, trenchMaxX,
            0,
            FieldConstants.RightTrench.openingWidth,
            ROTATION_EARLY_TRIGGER, 0.0);

        boolean inOppLeftTrench = poseInZone(robotPose,
            oppTrenchMinX, oppTrenchMaxX,
            FieldConstants.fieldWidth - FieldConstants.LeftTrench.openingWidth,
            FieldConstants.fieldWidth,
            ROTATION_EARLY_TRIGGER, 0.0);

        boolean inOppRightTrench = poseInZone(robotPose,
            oppTrenchMinX, oppTrenchMaxX,
            0,
            FieldConstants.RightTrench.openingWidth,
            ROTATION_EARLY_TRIGGER, 0.0);

        return inLeftTrench || inRightTrench || inOppLeftTrench || inOppRightTrench;
    }
    ...
    private boolean inBumpZone() {
        ...
        double bumpMinX = FieldConstants.LinesVertical.hubCenter - FieldConstants.LeftBump.width / 2;
        double bumpMaxX = FieldConstants.LinesVertical.hubCenter + FieldConstants.LeftBump.width / 2;

        double oppBumpMinX = FieldConstants.LinesVertical.oppHubCenter - FieldConstants.LeftBump.width / 2;
        double oppBumpMaxX = FieldConstants.LinesVertical.oppHubCenter + FieldConstants.LeftBump.width / 2;

        boolean inLeftBump = poseInZone(robotPose,
            bumpMinX, bumpMaxX,
            FieldConstants.LinesHorizontal.leftBumpEnd,
            FieldConstants.LinesHorizontal.leftBumpStart,
            0.0, 0.0);

        boolean inRightBump = poseInZone(robotPose,
            bumpMinX, bumpMaxX,
            FieldConstants.LinesHorizontal.rightBumpEnd,
            FieldConstants.LinesHorizontal.rightBumpStart,
            0.0, 0.0);

        boolean inOppLeftBump = poseInZone(robotPose,
            oppBumpMinX, oppBumpMaxX,
            FieldConstants.LinesHorizontal.leftBumpEnd,
            FieldConstants.LinesHorizontal.leftBumpStart,
            0.0, 0.0);

        boolean inOppRightBump = poseInZone(robotPose,
            oppBumpMinX, oppBumpMaxX,
            FieldConstants.LinesHorizontal.rightBumpEnd,
            FieldConstants.LinesHorizontal.rightBumpStart,
            0.0, 0.0);
    ...
    private Rotation2d getBumpLockAngle() {
        for (int i = -135; i < 180; i+= 90) {
            if (Math.abs(MathUtil.inputModulus(drive.getState().Pose.getRotation().getDegrees() - i, -180, 180)) <= 45) {
    ...
    @Override
    public void execute() {
        Translation2d linearVel = getLinearVelocityFromJoysticks(xSupplier.getAsDouble(), ySupplier.getAsDouble());
        ...
        switch (currentDriveMode) {
           	case NORMAL:
               	    double rawRot = MathUtil.applyDeadband(rotSupplier.getAsDouble(), DriveConstants.DEADBAND);
           	...
           	case TRENCH_LOCK:
          		...
          		targetY = trenchYController.calculate(drive.getState().Pose.getY());
                if (trenchYController.atSetpoint()) targetY = 0;

                rotationController.setSetpoint(getTrenchLockAngle().getRadians());
                rot = rotationController.calculate(drive.getState().Pose.getRotation().getRadians());
